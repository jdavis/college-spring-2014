...
\begin{homeworkProblem}
    ...
    Suppose you roll this die 33 times. Let \(S\) be the sum of these 33 roled
    values and let \(Y\) be the random variable that represents the mean of
    these 33 values minus the overal mean of 10.5:
    \[
        Y = \frac{S}{33} - 10.5
    \]
    ...
    Determine the probability (using CLT) that:

    \begin{enumerate}[(a)]
        \item \(Y\) is less than -0.5
        \item \(Y\) is less than -2.5
    \end{enumerate}
...
\begin{homeworkProblem}
    ...
    of a small corporation is 30 calls per hour. Assume that the arriving calls follows
    a Poisson process.
    ...
    What is the probability that four or mroe calls will arrive in a 5-minute interval?
    \\
    ...
    What is the probability that one minute will pass before the next incoming call?
    \\
...
\begin{homeworkProblem}
    ...
    \begin{enumerate}
        \item The customers arrive according to a Poisson process. The average
            time between arrivals is 30 minutes.
    ...
    Draw a state diagram with possible states and corresponding birth/death rates. Since
    there are an infinite number of states, show enough states to show the pattern or arrival
    and service rates.
    \\
    ...
    Let \(X\) be the number of customers in the store. Determine the "large t"
    probability function for the different states of \(X\). What probability
    distribution is this, and what is the value of the parameter in that
    distribution?
    \\
