...
@
    ...
    \[
        \begin{split}
            ...
            &= \sum_{k = 1}^{\infty} \Pr(X \geq k) \Pr_Y(k)
            \\
            &= \sum_{k = 1}^{\infty} (1 - \Pr(X < k)) \Pr_Y(k)
            \\
            &= \sum_{k = 1}^{\infty} (1 - (1 - (1 - p_x)^k)) \Pr_Y(k)
            \\
            &= \sum_{k = 1}^{\infty} (1 - p_x)^k \Pr_Y(k)
            \\
            &= \sum_{k = 1}^{\infty} (1 - p_x)^k \cdot p_y(1 - p_y)^{k - 1}
        \end{split}
    \]

    Rearranging and simplifying gives us the following:
    \[
        \begin{split}
            \Pr(X \geq Y)
            &= \sum_{k = 1}^{\infty} p_y(1 - p_x)^k (1 - p_y)^{k - 1}
            \\
            &= \sum_{k = 1}^{\infty} (0.1) (1 - 0.2)^k (1 - 0.1)^{k - 1}
            \\
            &= (0.1) \sum_{k = 1}^{\infty} (0.8)^k (0.9)^{k - 1}
            \\
            &= (0.1) \sum_{k = 1}^{\infty} (0.8) (0.8)^{k - 1} (0.9)^{k - 1}
            \\
            &= (0.08) \sum_{k = 1}^{\infty} (0.72)^{k - 1}
        \end{split}
    \]
