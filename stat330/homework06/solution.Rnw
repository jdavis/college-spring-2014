...
@
    ...
    \[
        \begin{split}
            ...
            \\
            &= \sum_{k = 1}^{\infty} p_y(1 - p_x) (1 - p_x)^{k - 1} (1 - p_y)^{k - 1}
            \\
            &= p_y(1 - p_x) \left[1 + (1 - p_x)^{1} (1 - p_y)^{1} + (1 - p_x)^{2} (1 - p_y)^{2} + \ldots \right]
            \\
            &= p_y(1 - p_x) \left( \frac{1 - x}{x} \right) \left( \frac{1 - y}{y} \right)
        \end{split}
    \]

    Our final equation is below and substituting our values gives us:
    \[
        \begin{split}
            \Pr(X \geq Y)
            &= \frac{
