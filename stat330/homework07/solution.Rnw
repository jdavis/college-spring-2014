...
\begin{homeworkProblem}
    ...
    matrix. This gives us the following where \(P^{k + 1}\) indicates the
    ...
    \[
        \text{0 students} = 0.0625, \quad
        \text{1 student} = 0.5, \quad
        \text{2 students} = 0.4375
    \]
...
\begin{homeworkProblem}
    ...
    8 minutes. This gives us \(t = 8\) then \(\lambda t = 4\), we know that
    there is an Exponential distribution between each task. This means that if
    we let \(X = \text{\# of tasks in interval 8 minutes}\), then \(X
    \dist{Poisson}(4)\) and we want \(\Pr(X = 5)\) which is:
    \[
        \Pr(X = 5)
