...
\begin{homeworkProblem}
    ...
    Since we want the time between events, if we let \(T\) be the random
    variable for time between jobs, then we know that \(T
    \dist{Exponential}(\lambda)\). Since we know \(\lambda = 0.5\), we get:
    \(T \dist{Exponential}(.5)\). The cdf is: \(F(x) = 1 - e^{-\lambda x}\).
    \\

    We want to find the probability that \(T\) is less
