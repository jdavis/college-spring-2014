...
@
    ...
    This suggests that \(X_1\) and \(X_2\) are pretty colinear. Let's visualize
    this by using a scatter plot:
...
@
    ...
    This gives us the estimator \(\hat{\beta_0} = \Sexpr{fit.x1.coef[1]}\).
...
@
    ...
    This gives us the estimator \(\hat{\beta_0} = \Sexpr{fit.x2.coef[1]}\).
    Looking at the significance of this estimator:
...
@
    ...
    This is one of the "woes" of interpreting regression coefficients according
    to \textit{Data Analysis \& Regression} by Mosteller and Tukey, 1977.
    \\
...
<<p1g1, fig.pos="H", fig.cap="X1 and X2 Plots.">>=
par(mfrow = c(2, 2))
plot(new.fit)
@

    Based on this, the added point looks to be a outlier because the Cook's
    Distance is > 1.
...
<<p1g2, fig.pos="H", fig.cap="X1 Plots.">>=
par(mfrow = c(2, 2))
plot(new.fit.x1)
@
...
<<p1g3, fig.pos="H", fig.cap="X2 Plots.">>=
par(mfrow = c(2, 2))
plot(new.fit.x2)
@
...
# Predicted classes
classes <- rep("P-Down", dim(Weekly)[1])
classes[pred.glm >= 0.5] <- "P-Up"
@
...
@
    Giving us the fraction of: \((54 + 557)/(54 + 430 + 48 + 557) = 0.561\).
    \\

    The confusion matrix is telling us how many predicted classes were right
    and how many were wrong.  P-Down is the predicted down class and P-Up is
    the predicted up class. Therefore when the classes match, it means it was
...
classes <- rep("P-Down", dim(Weekly.test)[1])
classes[pred.glm.lag2 >= 0.5] <- "P-Up"
@
...
<<>>=
# Make matrix
table(Weekly.test$Direction, classes)
@

    Giving us the fraction of: \((9 + 56)/(9 + 34 + 5 + 56) = 0.625\).
    \\
...
<<>>=
# Make matrix
table(Weekly.test$Direction, pred.lda$class)
@
