library(MASS)
C1 <- mvrnorm(100, c(0,4), matrix(c(1,.1,.1,1),2))
C2 <- mvrnorm(100, c(-2,-2), matrix(c(1,.1,.1,1),2))
C3 <- mvrnorm(100, c(3,0), matrix(c(1,.1,.1,1),2))
C4 <- rbind(C1,C2,C3)
C <- data.frame(X1=C4[,1], X2=C4[,2])
C.class <- as.factor(rep(c("a","b","c"),each=100))
...
library(klaR)
partimat(C.class~C[,2]+C[,1],data=C,method="lda",prec=500)
