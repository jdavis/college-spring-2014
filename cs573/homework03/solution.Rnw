...
\begin{homeworkProblem}
    Describe how you can efficiently solve the least squares linear system \(\) (i.e., by
    not calculating an interverse) where \(\) has \(p\) linearly independent columns, \(\beta \in\)
    and \(\)?
    \\

    \textit{Hint:} Think in terms of matrix decompositions (it's not SVD!). Use
    Wikipedia.  \end{homeworkProblem}
    \\
