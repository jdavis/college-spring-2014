...
<<>>=
lambda <- seq(0, 10000)
fit.ridge <- lm.ridge(medv ~ ., data = Boston, lambda = lambda, alpha = 0)
...
# Find minimum
fit.matrix <- as.matrix(fit.ridge$GCV)
m <- which.min(fit.matrix)

fit.ridge$lambda[m]

abline(v = fit.ridge$lambda[m])

# Final result
#traceplot(results, X = c("lambda"))
#abline(v = results$kGCV)
